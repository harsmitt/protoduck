...
pub fn json_to_proto(state: &DescriptorPoolState, json: &str, message_type: &str) -> Result<Vec<u8>> {
...
fn json_value_to_field_value(value: &JsonValue, field: &FieldDescriptor) -> Result<Value> {
    ...
    if field.is_map() {
        ...
        for (key, json_value) in object {
            ...
            let map_value = json_value_to_scalar_value(json_value, &value_field, value_field.kind())?;
...
fn json_value_to_scalar_value(value: &JsonValue, field: &FieldDescriptor, kind: Kind) -> Result<Value> {
    match kind {
        Kind::Bool => value.as_bool().map(Value::Bool).ok_or_else(|| invalid_value(field, "boolean", value)),
        Kind::String => value.as_str().map(|v| Value::String(v.to_string())).ok_or_else(|| invalid_value(field, "string", value)),
        Kind::Bytes => {
            let encoded = value.as_str().ok_or_else(|| invalid_value(field, "base64 string", value))?;
            let bytes = base64::engine::general_purpose::STANDARD.decode(encoded).map_err(|err| ProtoDuckError::InvalidFieldValue {
        ...
        Kind::Int32 | Kind::Sint32 | Kind::Sfixed32 => {
            let n = i32::try_from(json_i64(value, field)?).map_err(|_| invalid_value(field, "int32", value))?;
            Ok(Value::I32(n))
        }
        Kind::Int64 | Kind::Sint64 | Kind::Sfixed64 => Ok(Value::I64(json_i64(value, field)?)),
        Kind::Uint32 | Kind::Fixed32 => {
            let n = u32::try_from(json_u64(value, field)?).map_err(|_| invalid_value(field, "uint32", value))?;
            Ok(Value::U32(n))
        }
        Kind::Uint64 | Kind::Fixed64 => Ok(Value::U64(json_u64(value, field)?)),
        Kind::Float => {
            let n = json_f64(value, field)? as f32;
            if !n.is_finite() { return Err(invalid_value(field, "finite float", value)); }
            Ok(Value::F32(n))
        }
        Kind::Double => {
            let n = json_f64(value, field)?;
            if !n.is_finite() { return Err(invalid_value(field, "finite double", value)); }
            Ok(Value::F64(n))
        }
        Kind::Enum(enum_descriptor) => match value {
            JsonValue::String(name) => enum_descriptor.get_value_by_name(name)
            ...
            JsonValue::Number(_) => {
                let n = i32::try_from(json_i64(value, field)?).map_err(|_| invalid_value(field, "enum number", value))?;
        ...
        Kind::Message(message_descriptor) => {
            ...
            for (nested_name, nested_value) in object {
                let nested_field = message_descriptor.get_field_by_name(nested_name).ok_or_else(|| {
                    ProtoDuckError::FieldNotFound(nested_name.clone(), message_descriptor.full_name().to_string())
                })?;
...
fn json_string_to_map_key(key: &str, field: &FieldDescriptor) -> Result<MapKey> {
    match field.kind() {
        Kind::Bool => key.parse::<bool>().map(MapKey::Bool).map_err(|_| invalid_map_key(field, key)),
        Kind::Int32 | Kind::Sint32 | Kind::Sfixed32 => key.parse::<i32>().map(MapKey::I32).map_err(|_| invalid_map_key(field, key)),
        Kind::Int64 | Kind::Sint64 | Kind::Sfixed64 => key.parse::<i64>().map(MapKey::I64).map_err(|_| invalid_map_key(field, key)),
        Kind::Uint32 | Kind::Fixed32 => key.parse::<u32>().map(MapKey::U32).map_err(|_| invalid_map_key(field, key)),
        Kind::Uint64 | Kind::Fixed64 => key.parse::<u64>().map(MapKey::U64).map_err(|_| invalid_map_key(field, key)),
...
fn json_i64(value: &JsonValue, field: &FieldDescriptor) -> Result<i64> {
    if let Some(n) = value.as_i64() { return Ok(n); }
    value.as_str().ok_or_else(|| invalid_value(field, "integer or integer string", value))?.parse::<i64>().map_err(|_| invalid_value(field, "integer or integer string", value))
}

fn json_u64(value: &JsonValue, field: &FieldDescriptor) -> Result<u64> {
    if let Some(n) = value.as_u64() { return Ok(n); }
    value.as_str().ok_or_else(|| invalid_value(field, "unsigned integer or integer string", value))?.parse::<u64>().map_err(|_| invalid_value(field, "unsigned integer or integer string", value))
}

fn json_f64(value: &JsonValue, field: &FieldDescriptor) -> Result<f64> {
    value.as_f64().ok_or_else(|| invalid_value(field, "number", value))
}

fn invalid_value(field: &FieldDescriptor, expected: &str, value: &JsonValue) -> ProtoDuckError {
    ProtoDuckError::InvalidFieldValue { field: field.name().to_string(), expected: expected.to_string(), actual: json_type(value).to_string() }
}

fn invalid_map_key(field: &FieldDescriptor, key: &str) -> ProtoDuckError {
    ProtoDuckError::InvalidFieldValue { field: field.name().to_string(), expected: "valid protobuf map key".to_string(), actual: key.to_string() }
}
