...
fn json_value_to_message(
    ...
    for (field_name, json_value) in object {
        let field = descriptor
            ...
            .ok_or_else(|| ProtoDuckError::FieldNotFound(
                field_name.clone(),
...
fn json_value_to_scalar_value(
    ...
    match kind {
        ...
        Kind::Int32 | Kind::Sint32 | Kind::Sfixed32 => {
            let n = json_i64(value, field)?;
            let n = i32::try_from(n).map_err(|_| invalid_value(field, "int32", value))?;
            Ok(Value::I32(n))
        }

        Kind::Int64 | Kind::Sint64 | Kind::Sfixed64 => {
            Ok(Value::I64(json_i64(value, field)?))
        }

        Kind::Uint32 | Kind::Fixed32 => {
            let n = json_u64(value, field)?;
            let n = u32::try_from(n).map_err(|_| invalid_value(field, "uint32", value))?;
            Ok(Value::U32(n))
        }

        Kind::Uint64 | Kind::Fixed64 => {
            Ok(Value::U64(json_u64(value, field)?))
        }
        ...
        Kind::Enum(enum_descriptor) => match value {
            ...
            JsonValue::Number(_) => {
                let n = json_i64(value, field)?;
                let n = i32::try_from(n).map_err(|_| invalid_value(field, "enum number", value))?;
...
fn json_i64(value: &JsonValue, field: &FieldDescriptor) -> Result<i64> {
    ...
    // Accept strings as well so int64/uint64 JSON following the canonical
    // protobuf JSON convention can be consumed in addition to proto_to_json's
